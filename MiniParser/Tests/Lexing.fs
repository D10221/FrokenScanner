...
let Test1() =
    let result = Scan [ '\r'; '\n' ]
    equals ("\r\n", "newline", 0, 0) result.[0]

[<Fact>]
let Test2() =
    let r = (Scan [ ' '; '\n' ])
    equals (" ", "space", 0, 0) r.[0]
    equals ("\n", "newline", 1, 0) (Scan [ ' '; '\n' ]).[1]
...
let Test3() =
    let result = Scan [ '\n'; ' ' ]
    equals ("\n", "newline", 0, 0) result.[0]
    equals (" ", "space", 1, 1) result.[1]

[<Fact>]
let Test4() =
    let result = Scan [ '\r'; '\r'; '\n'; '\r' ]
    equals ("\r", "newline", 0, 0) result.[0]
    equals ("\r\n", "newline", 1, 1) result.[1]
    equals ("\r", "newline", 3, 2) result.[2]

[<Fact>]
let Test5() =
    let result = Scan [ '1'; '1'; 'a'; 'a' ]
    equals ("11", "number", 0, 0) result.[0]
    equals ("aa", "word", 2, 0) result.[1]
...
let Test7() =
    ...
    equals ("aB1@$#_", "word", 1, 0) result.[1]
    equals ("!", "symbol", 8, 0) result.[2]
...
[<Fact>]
let LineCounts() =
    let actual = (Scan [ 'a'; '\n'; 'a'; '\n'; 'a'; '\n' ]) |> List.map (fun (_, _, _, lineNo) -> lineNo)
